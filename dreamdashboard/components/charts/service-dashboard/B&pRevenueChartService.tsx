...
interface BpRevenueChartServiceProps {
  // Add any additional props needed for the chart
  currentData: any;
}

const BpRevenueChartService: React.FC<BpRevenueChartServiceProps> = (
  props: any
) => {
  // useEffect(() => {
  ...
  // console.log(currentData,'props in b&p revvv');

  const series = [
    props?.currentData?.find((data: any) => data.kpi_code == "bp_parts")
      ?.kpi_result === null
      ? 0
      : Number(
          props?.currentData?.find((data: any) => data.kpi_code == "bp_parts")
    ...
    props?.currentData?.find((data: any) => data.kpi_code == "bp_labour")
      ?.kpi_result === null
      ? 0
      : Number(
          props?.currentData?.find((data: any) => data.kpi_code == "bp_labour")
              ...
              formatter: function () {
                const parts_bp_value =
                  props?.currentData?.find(
                    (data: any) => data.kpi_code == "bp_parts"
                  )?.kpi_result === null
                    ? "NDF"
                    : Number(
                        props?.currentData?.find(
                ...
                const labour_bp_value =
                  props?.currentData?.find(
                    (data: any) => data.kpi_code == "bp_labour"
                  )?.kpi_result === null
                    ? "NDF"
                    : Number(
                        props?.currentData?.find(
...
export default BpRevenueChartService;
